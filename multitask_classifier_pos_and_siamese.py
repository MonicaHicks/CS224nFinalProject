...
# from pytorch.utilities import CombinedLoader
...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        """
        Siamese Run
        self.sst_linear = nn.Linear(config.hidden_size, N_SENTIMENT_CLASSES) 
        self.para_linear = nn.Linear(config.hidden_size * 3, 1)
        self.sts_linear = nn.Linear(config.hidden_size * 3, 1)
        """

        """
        Siamese + POS Run
        """
        self.sst_linear = nn.Linear(config.hidden_size * 2, N_SENTIMENT_CLASSES) 
        self.para_linear = nn.Linear(config.hidden_size * 3 * 2, 1)
        self.sts_linear = nn.Linear(config.hidden_size * 3 * 2, 1)

        """
        Regular run
        self.sst_linear = nn.Linear(config.hidden_size, N_SENTIMENT_CLASSES) 
        self.para_linear = nn.Linear(config.hidden_size + self.hidden_size, 1)
        self.sts_linear = nn.Linear(config.hidden_size + self.hidden_size, 1)
        """
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        return self.sst_linear(concatenated_embeddings)
    ...
    def predict_paraphrase(self,
        ...
        pos_tag_ids_1 = [[pos2idx.get(tag, 0) for _, tag in sent_pos_tags] for sent_pos_tags in pos_tags_list_1]
        ...
        pos_tag_ids_2 = [[pos2idx.get(tag, 0) for _, tag in sent_pos_tags] for sent_pos_tags in pos_tags_list_2]
        ...
        siamese = torch.cat((concatenated_embeddings_1 , concatenated_embeddings_2), 1)
        siamese = torch.cat((siamese, torch.abs(concatenated_embeddings_1 - concatenated_embeddings_2)))
        return self.para_linear(siamese)
    ...
    def predict_similarity(self,
        ...
        siamese = torch.cat((concatenated_embeddings_1, concatenated_embeddings_2), 1)
        siamese = torch.cat((siamese, torch.abs(concatenated_embeddings_1 - concatenated_embeddings_2)))
        return self.sts_linear(siamese)
...
def train_multitask(args):
    ...
    # iterables = {'sst': sst_train_dataloader,
    ...
    for epoch in range(args.epochs):
        ...
        for sts_batch in tqdm(sts_train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE):
            ...
            # print("para", para_batch)
            b_ids_1, b_ids_2, b_mask_1, b_mask_2, b_labels = (para_batch['token_ids_1'], para_batch['token_ids_2'],
                                        para_batch['attention_mask_1'],para_batch['attention_mask_2'], para_batch['labels'])
            ...
            para_loss = lossfx(logits[:,0].to(torch.float32), b_labels.view(-1).to(torch.float32)) / args.batch_size
            ...
            # print("sts", sts_batch)
            b_ids_1, b_ids_2, b_mask_1, b_mask_2, b_labels = (sts_batch['token_ids_1'], sts_batch['token_ids_2'],
                                        sts_batch['attention_mask_1'],sts_batch['attention_mask_2'], sts_batch['labels'])
            ...
            sts_loss = lossfx(logits[:,0].to(torch.float32), b_labels.view(-1).to(torch.float32)) / args.batch_size
