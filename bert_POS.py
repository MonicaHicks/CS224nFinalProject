...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    # get POS ids (array index corresponding to tag types at top of file)
    pos_tag_ids = self.generate_pos_ids(input_ids)
    # make embedding from the tag
    pos_tags_embeds = self.pos_tags_embedding(pos_tag_ids)
    # concatenate the word embedding and POS tag embedding
    concatenated_embeddings = torch.cat((final_embedding, pos_tags_embeds), dim=-1)
    # resize it back to the size of the word embedding
    resized_embeddings = self.projection(concatenated_embeddings)

    final_embedding = self.embed_layer_norm(resized_embeddings)
  ...
  def generate_pos_ids(self, input_ids):
    # get original tokens
    sentences = [self.convert_ids_to_tokens(ids) for ids in input_ids.tolist()]
    # stores the part of speech id for teach token
    # where the id is the index in pos_tags at top of file
    all_pos_ids = []
    for tokens in sentences:
      # get part of speech for otkens
      pos_tags = pos_tag(tokens)
      # get the POS identifier from tags at top of file
      pos_ids = [pos2idx.get(tag, 0) for _, tag in pos_tags]
      # add these ids to our list
      all_pos_ids.append(pos_ids)
    # pad to longest sentence
    max_len = max(len(pos_ids) for pos_ids in all_pos_ids)
    # get padded representations
    all_pos_ids = [pos_ids + [0] * (max_len - len(pos_ids)) for pos_ids in all_pos_ids]
    # return as tensor
    return torch.tensor(all_pos_ids, dtype=torch.long, device=input_ids.device)

  def convert_ids_to_tokens(self, ids):
    # uses toeknizer from tokenizer.py
    tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")
    # returns tokens based on id
