...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.sts_linear = nn.Linear(self.hidden_size, 1)
    ...
    def predict_similarity(self,
        ...
        output_1 = self.sts_linear(sent_1) #.to(attention_mask_1.device)
        output_2 = self.sts_linear(sent_2) #.to(attention_mask_1.device)

        # lossfx = torch.nn.CosineEmbeddingLoss()
        cos_sim = torch.nn.CosineSimilarity()
        output = cos_sim(output_1, output_2)
        # print(output)
        output = (output + 1)*(5/2)
        # print(output)
        # print(output.shape)
...
def train_multitask(args):
    ...
    for epoch in range(args.epochs):
        ...
        for sts_batch in tqdm(sts_train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE):
            ...
            optimizer.zero_grad()
            # logits_1, logits_2 = model.predict_similarity(b_ids_1, b_mask_1, b_ids_2,b_mask_2)
            ...
            # print(b_labels)
            # sts_loss = lossfx(logits_1[:,0].to(torch.float32), logits_2[:,0].to(torch.float32), b_labels.view(-1).to(torch.float32)) / args.batch_size
            sts_loss = lossfx(logits.to(torch.float32), b_labels.view(-1).to(torch.float32)) / args.batch_size
